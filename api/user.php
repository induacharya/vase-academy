...
$app->post('/sessionValidate', function ($request, $response, $args) {
    $result =  new stdClass();
    $param =  $request->getParsedBody();
    $userName = $param['userName'];
    $token = $param['token'];
    ...
    $Database=new Database();
    $Database->query("SELECT count(id) as count,token,id FROM login WHERE userName=:userName AND token=:token AND type=:type");
    
    $DBParameters["userName"]=$userName;
    $DBParameters["token"]=$token;
    $DBParameters["type"]=1;
    ...
    if( $QryOutput['count']  == 1 && $token != 'tokenCon'){
        $result->status = true ;
...
$app->post('/forgotPassword', function ($request, $response, $args) {
    ...
    $Database=new Database();
    $Database->query("SELECT count(id) as count,token,id FROM login WHERE email=:email AND type=:type");
    
    $DBParameters["email"]=$email;
    $DBParameters["type"]=1;
